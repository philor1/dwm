...
dragfact(const Arg *arg)
{
	...
	if (reshintsoffondrag == 1) {
		...
		if (ams == 1) {
			if (horizontal) {
				XWarpPointer(dpy, None, root, 0, 0, 0, 0, xbutt, m->wy + (int) ((float) (m->wh - gapp) * fact) + gapp/2);
				ybutt = m->wy + (int) ((float) (m->wh - gapp) * fact) + gapp/2;
			} else {
				XWarpPointer(dpy, None, root, 0, 0, 0, 0, m->wx + (int) ((float) (m->ww - gapp) * fact) + gapp/2, ybutt);
				xbutt = m->wx + (int) ((float) (m->ww - gapp) * fact) + gapp/2;
			}
		}
		if (am == 2) {
			if (m->ltaxis[1] == 2) {
				XWarpPointer(dpy, None, root, 0, 0, 0, 0, xbutt, mn->y - gapp/2);
				ybutt = mn->y - gapp/2;
			} else {
				XWarpPointer(dpy, None, root, 0, 0, 0, 0, mn->y - gapp/2, ybutt);
				xbutt = mn->y - gapp/2;
			}
		}
		if (as == 2) {
			if (m->ltaxis[2] == 2) {
				XWarpPointer(dpy, None, root, 0, 0, 0, 0, xbutt, sn->y - gapp/2);
				ybutt = sn->y - gapp/2;
			} else {
				XWarpPointer(dpy, None, root, 0, 0, 0, 0, sn->y - gapp/2, ybutt);
				xbutt = sn->y - gapp/2;
			}
		}
	}
	...
	do {
		...
		case MotionNotify:
			...
			if (ams == 1) {
				if (horizontal)
					fact = (float) (diffy) / (float) (m->wh - 3 * gapp);
				else
					fact = (float) (diffx) / (float) (m->ww - 3 * gapp);
				if (mirror)
					fact = 1.0 - fact;
				if (fact && m->mfact + fact < 0.9 && m->mfact + fact > 0.1)
					selmon->mfact = selmon->pertag->mfacts[selmon->pertag->curtag] += fact;
			}
			if (as == 2) {
				if (m->ltaxis[2] == 2) {
					fact = (float) (diffy) * (float) (sfactn + sfactp) / (float) (sn->h + sp->h);
					if ((sn->cfact - fact) > 0.25 && (sp->cfact + fact) > 0.25
						&& (int) (ev.xmotion.y - ybutt) <= sn->h && (int) (ybutt - ev.xmotion.y) <= sp->h) {
							sn->cfact -= fact;
							sp->cfact += fact;
					}
				} else {
					fact = (float) (diffx) * (float) (sfactn + sfactp) / (float) (sn->w + sp->w);
					if ((sn->cfact - fact) > 0.25 && (sp->cfact + fact) > 0.25
						&& (int) (ev.xmotion.x - xbutt) <= sn->w && (int) (xbutt - ev.xmotion.x) <= sp->w) {
							sn->cfact -= fact;
							sp->cfact += fact;
					}
				}
			...
			if (am == 2) {
				if (m->ltaxis[1] == 2) {
					fact = (float) (diffy) * (float) (mfactn + mfactp) / (float) (mn->h + mp->h);
					if ((mn->cfact - fact) > 0.25 && (mp->cfact + fact) > 0.25
						&& (int) (ev.xmotion.y - ybutt) <= mn->h && (int) (ybutt - ev.xmotion.y) <= mp->h) {
							mn->cfact -= fact;
							mp->cfact += fact;
					}
				} else {
					fact = (float) (diffx) * (float) (mfactn + mfactp) / (float) (mn->w + mp->w);
					if ((mn->cfact - fact) > 0.25 && (mp->cfact + fact) > 0.25
						&& (int) (ev.xmotion.x - xbutt) <= mn->w && (int) (xbutt - ev.xmotion.x) <= mp->w) {
							mn->cfact -= fact;
							mp->cfact += fact;
					}
				}
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (m->tagset[m->seltags] & 1 << i) {
			...
			drw_text(drw, x, 0, w, bh - (bartheme ? 1 : 0), lrpad / 2, tags[i], urg & 1 << i);
			if (bartheme) {
...
drawebar(char* stext, Monitor *m, int xpos)
{
	...
	while (stext[++k]) {
		...
		if ((unsigned char)stext[k] < ' ') {
			...
			if (sep == esep && block == eblock && block) {
				...
				drw->scheme[ColFg] = scheme[(bartheme ? SchemeUnfocus : SchemeBar)][ColFg];
				drw->scheme[ColBg] = scheme[(bartheme ? SchemeUnfocus : SchemeBar)][ColBg];
			}
			...
			while (text[++i]) {
				if (text[i] == '^' && !isCode) {
					...
					text[i] = '\0';
					w = TEXTW(text) - lrpad;
					drw_text(drw, x, bartheme ? sep != esep || block != eblock ? -1 : 0 : 0, w, bh, 0, text, 0);
					x += w;
					/* process code */
					while (text[++i] != '^') {
						if (text[i] == 'c') {
	...
	if (bartheme) {
		drw_setscheme(drw, scheme[LENGTH(colors)]);
		drw->scheme[ColFg] = statuslastblock == 1 ? scheme[SchemeFocus][ColFg] : scheme[SchemeBar][ColFg];
		drw->scheme[ColBg] = statuslastblock == 1 ? scheme[SchemeFocus][ColBg] : scheme[SchemeBar][ColFloat];
		drw_text(drw, x, statuslastblock == 1 ? -1 : 0, w, bh, 0, text, 0);
		if (statuslastblock == 1 && x + w - sep > 0)
			drawtheme(sep, x + w - sep, 2, statustheme);
	} else {
		drw_setscheme(drw, scheme[SchemeBar]);
...
void drawbartab(Monitor *m, Client *c, int x, int w, int tabgroup_active) {
	...
	if (oneclientdimmer == 1) {
		...
		if (n == 1) {
			...
			drw->scheme[ColBg] = scheme[SchemeBar][bartheme ? ColFloat : ColBg];
			drw_text(drw, x, 0, w, bh - (bartheme ? 1 : 0), lrpad / 2, c->name, 0);
		}
	}
	if (n != 1) {
		drw_setscheme(drw, scheme[m->sel == c ? SchemeSelect : SchemeUnfocus]);
		drw_text(drw, x, 0, w, bh - (bartheme ? 1 : 0), lrpad / 2, c->name, 0);
		if (bartheme && tabbartheme) {
			drawtheme(x, w, m->sel == c ? 3 : 1, tabbartheme);
		} else {
...
// select 	= 3  floater 	= 2
	...
	if (theme == 2) {
		if (status == 1) {
			...
			XFillRectangle(drw->dpy, drw->drawable, drw->gc, x, bh - 2, x + s, 2);
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		wa.background_pixel = scheme[SchemeBar][bartheme ? ColFloat : ColBg].pixel;
		wa.border_pixel = 0;
		wa.colormap = cmap;
